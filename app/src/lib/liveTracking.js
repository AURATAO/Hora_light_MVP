 ...
 */
export function broadcastPhase({ isAssignee, status, hasOpenWorklog, sessionCount, enrouteAt }) {
  if (!isAssignee || status !== 'open') return 'none'
  if (hasOpenWorklog) return 'working'
  if (enrouteAt && (sessionCount ?? 0) === 0) return 'enroute'
  return 'none'
}
