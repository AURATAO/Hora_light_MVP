...
test("names the window in the server's words, and falls back to the shipped one", () => {
  assert.equal(
    surgeRateNote({ surge_rate: true, per_minute_rate_cents: 100, included_minutes: 15, surge_window: "9 PM–8 AM" }),
    "Evening & overnight rate: $1.00/min after the first 15 minutes (9 PM–8 AM).",
  );
  assert.equal(
