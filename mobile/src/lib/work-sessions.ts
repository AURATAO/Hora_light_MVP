 ...
 * anything, and never reaches the invoice.
 *
 ...
 * quote, untouched.
 *
 * `app/src/lib/workSessions.js` is the same logic for the web app. Metro
 * cannot import across that package boundary, so the two are kept in step by
 * hand and by test (app/src/lib/workSessions.test.mjs reads THIS file).
 * Change one, change the other.
 */

/** One entry in the rendered timeline: a session, or the free gap before it. */
export type TimelineEntry =
  ...
  | { kind: "gap"; startAt: string; endAt: string; minutes: number };

/** The wire-shape-agnostic input: one worklog, normalized. */
 ...
 */
export function sessionMinutes(startAt: string, endAt: string): number {
...
export function buildSessionTimeline(
  ...
  for (const entry of sorted) {
    ...
    // one.
    if (previousEnd) {
      const minutes = sessionMinutes(previousEnd, entry.startAt);
      if (minutes > 0) {
    ...
    timeline.push({
      ...
      minutes: running
        ...
        : sessionMinutes(entry.startAt, entry.endAt as string),
      running,
    });
...
export function isPaused({
  ...
  return status === "open" && sessionCount > 0 && !hasOpenWorklog;
}
 ...
 * cancelled task.
 *
 * Calm, and specific about the one thing they actually want to know
 * (DESIGN.md §6). A cancelled task used to disappear from their lists
 * entirely, and the notification behind it said "task cancelled, thanks for
 * your time" — which reads as "and you are getting nothing". Under the
 * cancellation billing policy that is usually false: the base fee is theirs
 * because they committed. The settlement card directly below this says the
 * number, so this line's job is only to say that there IS one and that
 * nothing is owed of them.
 */
...
export function showsWaitHint(category: string | undefined): boolean {
 ...
 */
export function gapsNote(timeline: readonly TimelineEntry[]): string | null {
